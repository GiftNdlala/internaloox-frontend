...
const WarehouseSideNav = ({ user, onLogout, activeTab, onTabChange, notifications = [], isMobile = false, isNavCollapsed = false, onNavToggle }) => {
  ...
  const sideNavContent = (
    <div className="warehouse-side-nav" style={{
      ...
      <div style={{
        ...
        <div style={{
          fontSize: isNavCollapsed ? '18px' : '24px',
          fontWeight: 'bold',
          color: '#3498db',
          marginBottom: '5px'
        }}>
          <FaWarehouse className="me-2" />
          {!isNavCollapsed && 'OOX Warehouse'}
        </div>
        {!isNavCollapsed && (
          <div style={{
            fontSize: '14px',
        ...
        {/* Toggle Button */}
        <div 
          onClick={() => onNavToggle && onNavToggle(!isNavCollapsed)}
          style={{
            position: 'absolute',
            top: '15px',
            right: '15px',
        ...
        >
          {isNavCollapsed ? <FaBars size={16} /> : <FaTimes size={16} />}
        </div>
      </div>

      {/* User Info */}
      <div style={{
        padding: isNavCollapsed ? '15px 10px' : '15px 20px',
        borderBottom: '1px solid #34495e',
        backgroundColor: '#34495e',
        flexShrink: 0,
            ...
            <Nav.Item key={item.id}>
              <Nav.Link
                ...
                style={{
                  ...
                  border: 'none',
                  padding: isNavCollapsed ? '15px 20px' : '15px 25px',
                  margin: '2px 15px',
                  borderRadius: '8px',
                  cursor: 'pointer',
                  transition: 'all 0.3s ease',
      ...
      <div style={{
        padding: isNavCollapsed ? '20px 10px' : '20px',
        ...
        >
          ...
          {!isNavCollapsed && <span style={{ fontSize: '14px' }}>Logout</span>}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{
      width: isNavCollapsed ? '80px' : '280px',
      height: '100vh',
      position: 'fixed',
      left: 0,
      top: 0,
